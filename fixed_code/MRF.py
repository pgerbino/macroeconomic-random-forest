...
class MacroRandomForest:
    ...
    def _ensemble_loop(self):
        ...
        return {"YandX": self.data.iloc[:, np.concatenate([self.y_pos, self.z_pos])],
    ...
    def _one_MRF_tree(self, b):
        ...
        # betas[:, 0] = beta_bank[:, 0]*self.std_stuff['std'].flat[self.y_pos] + \
            # self.std_stuff['mean'].flat[self.y_pos]
        betas[:, 0] = (beta_bank[:, 0].reshape(-1,1)*self.std_stuff['std'].flat[self.y_pos] + \
            self.std_stuff['mean'].flat[self.y_pos]).reshape(-1,)

        for kk in range(1, betas.shape[1]):

            betas[:, kk] = (beta_bank[:, kk].reshape(-1,1)*self.std_stuff['std'].flat[self.y_pos] / \
                self.std_stuff['std'].flat[self.z_pos[kk-1]]).reshape(-1,)
    ...
    def _splitter_mrf(self, x, y, z, whos_who, rando_vec, rw_regul_dat):
        ...
        """
        Internal method to perform the splitting operation for the MacroRandomForest model.
        ...
        z : array-like
            Additional feature matrix used for regularization.
        whos_who : array-like
            Array indicating the identity of observations.
        rando_vec : array-like
            Random vector used for random walk regularization.
        rw_regul_dat : array-like
            Data used for random walk regularization.

        Returns:
        --------
        list
            A list containing the minimum sum of squared errors (SSE), the chosen split value, 
            the coefficients of the linear model, and the number of observations in each split.
        """
